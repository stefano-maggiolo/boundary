...
BoundaryComputer::bt_m(int i)
{
  if (i < graph.K) // We have to assign m[i].
    ...
    {
      // If we decided all the marked points, we switch to assign values
  ...
  if (i < graph.K-1) // We have to assign a[i][j].
    {
      ...
      for (int n = start; n <= end; n++)
        {
          // We check the following, to ensure that $sum = G$ and
          // that all genus 0 curves are stabilized.
          if (j == graph.K-1)
            {
              ...
              // If we don't connect the vertex now...
              if (n == 0)
                {
                  // A vertex has to be connected to at least one
                  // different vertex.
                  if (graph.he[i] == graph.m[i]+2*graph.l[i])
                    continue;
                  // For i = K-1, we have the last chance to connect the 
                  // last vertex.
