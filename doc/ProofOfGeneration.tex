...
\begin{remark}
  ...
  N)$ with one vertex. Indeed, if there is one vertex, the choice of the genus  
  uniquely determines the number of loops on it after Definition 
  \ref{def:stable graph}.
\end{remark}
...
It is enough to generate the strict upper triangle of the matrix $a$ because the
matrix is symmetric, and the diagonal elements have already been generated in the 
vector $l$.
 
Every single datum is assigned trying all the possibilities within a range
that depends upon the values $G,N$, and upon the values of the data that have
already been filled.
 %Each entry of the matrix $M$ is filled with an integer in a specific range that we are going to describe in Section~\ref{sec:ranges}. For each of
...
The main ingredient to decide the lower endpoint of the range
is the concept of \emph{division\/} before an index $i$. 
We start filling the genus vector $g$ in a non decreasing way: every time we
assign a value $g_i$ strictly greater than $g_{i-1}$ we put a division before $i$.
Then we start filling the vector $n$ in such a way that, within two division ranges,
it is non decreasing. Again we introduce a division before $i$ every time
we assign a value $n_i$ strictly bigger than $n_{i-1}$. We follow this procedure
also for the vector $l$. Finally, we start filling the rows of the matrix $a$.
...
Once the divisions are fixed, for each datum we fix the range of possibilities
within it can vary. The range we introduce for each element to be chosen is
more refined than the one induced by the divisions, and it is explained in the 
following section. For each datum to be filled, the range depends upon $G$, $N$
and the data that have already been filled. The ranges are defined in such a way
that the conditions of Definition \ref{def:stable graph} are checked as early 
as possible.

%Finally, at the end of the $i$-th row of the matrix, we test that the $i$-th
...
that the graph is actually connected. Then we use the program Nauty \cite{nauty} 
to check if this graph is isomorphic to a previously generated graph.
If this is the case, we add the graph to the list of graphs of genus $G$
with $N$ marked points.

%sembrerebbe che se invece di usare il nostro programma e nauty
%in serie, i due fossero ottimizzati l'uno per l'altro si otterrebbero risultati 
%molto migliori?
\section{Description of the ranges}\label{sec:ranges}

In the previous section we have introduced the algorithm, by describing the
divisions. In this section we introduce accurate ranges for the possible values
of $g,n,l$ and $a$. In particular we try to obtain estimates on the upper 
extremum for each datum to be filled.
...
parameters and then by presenting a second list containing the extimates
that allow the definitions of the actual ranges.
...
\item amongst the $e^{\MAX}$ edges, there are necessarily $K-1$
  ...
  edges (not counting those that are needed to connect the graph) 
  is $2e^{\MAX} - K = 2(G - G_i) + K - 2$;
...
The following conditions define then the ranges for the possible choices 
for $n_i$:
...
\item we cannot assign more than $N$ marked points, hence (where we
  ...
  \end{align*}
\item if $g_i = 0$, for the purpose of stabilizing the first $i+1$ curves 
...
\item let $z$ be the index of the genus $0$ vertex with the least
  ...
  \begin{align*}
    ...
    %Qui togliendo il massimo sembrerebbe che si ottenga una condizione piu' debole
    %per l_i. Come mai non richiediamo piu' la condizione piu' forte nel secondo 
    %passaggio? sono in realte' uguali le due condizioni con e senza massimo?
    %Ho capito: il RHS della prima disuguaglianza è sempre maggiore o uguale a zero.
  \end{align*}
...
\begin{itemize}
\item in this section, the word ``stabilized'' goes back to its original
  meaning, i.e., it means ``with at least $3$ half edges'', because
  now we have already placed some non-loop edge;
  %Vuoi dire che fino ad ora stabile significava che c'erano 2 half edges
  %mentre ora ritornano ad essere 3? Se e' così non capisco il because nella 
  %frase precedente.
...
\item we need at least $K-2-c$ (if positive) edges to connect the
  ...
  hence:
  %Qui non capisco: devo rileggere la mail che mi hai mandato.
  \begin{align*}
...
class of graphs with $K$ vertices. The conditions of stability and of 
connectedness of the graph generated by the algorithm are checked 
separatedly so they are not really relevant in this section.
%Se e' vero quello che dico nel paragrafo successivo, va spiegato
%meglio di come l'ho fatto.
Moreover, as a consequence of the fact that our ranges for $g$,$n$,$l$ 
and $a$ are correct, we have that it is enough to prove the proposition
above in the case of an algorithm that works only with the ranges defined 
by divisions.
