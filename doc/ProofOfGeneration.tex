...
\begin{notation}
  ...
  represent the number of \emph{stabilizing\/} half edges, in the
  sense that they are not useless for the aim of stabilizing the
  curve. When deciding $a_{i,j}$, it is also useful to have defined
  $h_i$, the total number of half edges hitting the $i$-th
...
The following conditions limit the choices we have for $n_i$:
\begin{itemize}
...
\item we cannot assign more than $N$ marked points, hence (where we
  ...
  \end{align*}
\item if $g_i = 0$, to stabilize the first $i+1$ curves we cannot use
  marked points anymore, therefore we have
  \begin{align*}
    2 (i+1) - N^{(2)}_i - n^{(2)}_i &\leq (2(G - G_K) + K - 2)\\
    \Rightarrow n^{(2)}_i = \min(2, n_i) &\geq - (2(G - G_K) + K - 2) + (2(i+1) - N^{(2)}_i)\\
    \Rightarrow
    &\begin{cases}
...
\item assume $g_i = 0$; if $l_i > 0$, we are adding to the $i$-th
  ...
  \begin{align*}
    ...
    &\Rightarrow 2p_1 - N^{(2)} - (2-n^{(2)}_i) \max(0, 2-m_i) \leq 2(G - G_K - L_i - l_i + K - 1) - K\\
    &\Rightarrow 2l_i \leq 2(G - G_K - L_i) + K + N^{(2)} - n^{(2)}_i - 2p_i\,\text{.}
  \end{align*}
\item assume $g_i = 0$; after deciding $l_i$, we still have $e^{\MAX}$
  ...
  \begin{align*}
    ...
    &\Rightarrow n_i + 2l_i + G - G_K - L_i - l_i + K - 1 - 1 \geq 2\\
    &\Rightarrow l_i \geq 4 - n_i - G + G_K + L_i - K\,\text{.}
  \end{align*}
\end{itemize}
...
\begin{itemize}
\item in this section, the word ``stabilized'' returns to its original
  meaning, i.e., it means ``with at least $3$ half edges'', because
  now we already placed some non-loop edge;
\item $e^{\MAX} = G - G_K - L_K - A_{i,j} + K - 1$;
\item we already placed edges between $c$ couples of different
  vertices;
\end{itemize}


Here are the conditions that $a_{i,j}$ must satisfy:
\begin{itemize}
...
\item we need at least $K-2-c$ (if positive) edges to connect the
  ...
  \begin{align*}
    ...
    &\Rightarrow a_{i,j} \leq G - G_K - L_K - A_{i,j} +K - 1 - \max(0, K-2-c)\,\text{;}
  \end{align*}
\item $a_{i,j}$ contributes with at most $\max(0, 3-h_i) + \max(0,
  ...
  \begin{align*}
    ...
    &\Rightarrow 3p_1 - \sum_{g_{i^\prime} = 0} \min(3, n_i + 2l_i) - (\max(0, 3-h_i) + \max(0, 3-h_j)) &\leq 2 (G - G_K - L_K - A_{i,j} + K - 1 - a_{i,j})\\
    &\Rightarrow 2a_{i,j} \leq 2 (G - G_K - L_K - A_{i,j} + K - 1) - 3p_1 + \sum_{g_{i^\prime} = 0} \min(3, n_i + 2l_i) + \max(0, 3-h_i) + \max(0, 3-h_j)\,\text{.}
  \end{align*}
