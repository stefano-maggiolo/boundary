...
\begin{proof}
  ...
  $a_{i,j^\prime}$ is not a breaking position whenever $j^\prime<j$
  ...
  \begin{figure}[t]
    ...
    \caption{The matrix $a$ when the first breaking position is
      ...
      entries do not change.}\label{fig:a}
  \end{figure}
  ...
  % breaking. Otherwise, if the three vectors ($g$, $n$, and $l$) of
  % $G$ and $H$ coincide, let us compare $v(G)$ and $v(H)$, and take
  % the
  ...
  % $i^\prime = 0$:
  % \begin{itemize}
  % \item if $a_{0,j-1} > a_{0,j}$, then $a_{0,j}$ is indeed a
  %   breaking
  ...
  % j-2$, when we change strategy, comparing instead
  % $a_{j-1,i^\prime}$
  % with $a_{j,i^\prime}$, for $i^\prime > j$. The idea is then the
  % same: there is a trichotomy, one case conclude our proof, the
  % second cannot happen and the third force us to continue. But we
  % cannot continue till the end, because if we find always equality,
