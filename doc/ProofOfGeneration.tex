...
\begin{notation}
  ...
  %In this way the stability condition can be 
  %rephrased in: for every vertex $v$ of genus $0$, its number of half edges 
  %is at least $3$. %An half edge is said to come from an edge if it is of the
  %second type.
\end{notation}
...
\begin{itemize}
\item earlier in Notation \ref{not:partial_assign}, we observed that for the
...
\begin{table}[t]\label{tab:number}
  \begin{tabular}{rrrr}
    ...
    7 &  0 & 681 & 1,281,678\\
  \end{tabular}
  \caption{For small $G$, the maximum $N$ such that all stable graphs of type $(G,N)$ can be computed in less than $15$ minutes.}
\end{table}
